...
public class Connector extends LifecycleMBeanBase  {
    ...
    public Connector(String protocol) {
    	//setProtocol方法已经作废。以HTTP/1.1为例，由于默认情况下Apr不可用，
    	//所以protocolHandlerClassName会被设置为"org.apache.coyote.http11.Http11NioProtocol"，
    	//那么反射生成的protocolHandler就是Http11NioProtocol实例。Tomcat默认还会配置协议是AJP/1.3的Connector，
    	//那么此Connector的protocolHandler就是org.apache.coyote.ajp.AjpNioProtocol。
    ...
    public void setProtocol(String protocol) {
    	...
    	//apr是用jni调用底层，apr协议需要底层环境的支持 ； 一般默认没有apr环境的,所以为false
...
//        http://www.cnblogs.com/nizuimeiabc1/p/8672723.html
        //构造了CoyoteAdapter对象，并且将其设置为ProtocolHandler的Adapter。ProtocolHandler是做什么的呢？
        //Tomcat处理HTTP请求，需要有一个ServerSocket监听网络端口来完成任务。
        // Initialize adapter
        adapter = new CoyoteAdapter(this);
        //接口ProtocolHandler被设计成控制网络端口监听组件运行，负责组件的生命周期控制，这个接口实际并没有定义网络端口监听功能的规范，而是用于负责维护组件的生命周期。从ProtocolHandler的名字来看，它应该是网络协议的处理者，但它实际不负责这个功能，而是将其交给org.apache.coyote.Adapter来完成，这么设计估计是为了方便维护和拓展新功能。Http11Protocol是ProtocolHandler接口的一个实现(是Connector的默认处理协议),被设计用来处理HTTP1.1网络协议的请求,通过该类可以完成在某个网络端口上面的监听,同时以HTTP1.1的协议来解析请求内容，然后将请求传递到Connector所寄居的Container容器pipeline流水工作线上处理。此处的ProtocolHandler是何时生成的呢？还记得《TOMCAT源码分析——SERVER.XML文件的加载与解析》一文中的Digester和Rule吗？Digester在解析到<Connector>标签的时候，会执行startElement方法，startElement中会调用Rule的begin(String namespace, String name, Attributes attributes)方法，Connector对应的Rule包括ConnectorCreateRule，ConnectorCreateRule的begin方法的实现见代码
