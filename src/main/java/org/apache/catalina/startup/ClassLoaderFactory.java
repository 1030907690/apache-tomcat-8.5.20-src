...
public final class ClassLoaderFactory {
     ...
     */
    /**
      我们再进一步对 ClassLoaderFactory 进行分析，都是细节上的处理，比如利用文件路径构造带有明显协议的 URL 对象，
      例如本地文件的标准 URL 是 file:/D:/app.jar。另外，在创建 URLClassLoader 的时候还需要考虑 jdk 对权限控制的影响，
      因此 tomcat 利用 AccessController 创建 URLClassLoader，由此可见 tomcat 编码的严谨性。而我们在实际的开发过程中，
      有时候需要自定义类加载器，但往往不会考虑权限控制这块，所以在对类加载器进行编码时需要注意一下
    ...
    private static URL buildClassLoaderUrl(String urlString) throws MalformedURLException {
        ...
        String result = urlString.replaceAll("!/", "%21/"); // 转换成URL编码
