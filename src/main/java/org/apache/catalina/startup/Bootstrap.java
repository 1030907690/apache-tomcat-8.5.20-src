...
public final class Bootstrap {
    ...
    //在创建类加载器时，会读取相关的路径配置，并把路径封装成 Repository 对象，然后交给 ClassLoaderFactory 创建类加载器 https://blog.csdn.net/dwade_mia/article/details/80140585
    ...
    public void init() throws Exception {
     ...
     // 反射方法实例化Catalina，后面初始化Catalina用了很多反射，不知道意图是什么
        ...
        //实例化一个 org.apache.catalina.startup.Catalina 对象，并赋值给成员 catalinaDaemon，
   ...
   /**
    ...
    public static void main(String args[]) {
        ...
        if (daemon == null) {
            ...
            try {
            	//创建commonLoader、catalinaLoader、sharedLoader类加载器（默认情况下这三个类加载器指向同一个对象。建议看看 
            	//createClassLoader 方法，里面做的事情还挺多，比如装载 catalina.properties 里配置的目录下的文件和jar包，
            	//后两个加载器的父加载器都是第一个，最后注册了 MBean，可以用于 JVM 监控该对象）
                bootstrap.init();
            } catch (Throwable t) {
                handleThrowable(t);
                t.printStackTrace();
