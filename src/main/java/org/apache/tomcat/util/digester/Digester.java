...
public class Digester extends DefaultHandler2 {
     ...
     */
    //这个方法用来处理在 XML 文件中读到的内容。例如： 主要目的是获取 high 标签中的值
    @Override
    ...
    //当遇到Server的结束标签时，还会依次调用ObjectCreateRule、SetPropertiesRule及SetNextRule的end方法，不再赘述。所有元素的解析都与Server标签同理，最终将server.xml文件中设置的元素及其属性值，构造出tomcat中的容器，如：Server、Service、Connector等
    @Override
   ...
   //在遇到结束标签的时候，调用这个方法。
     ...
     * @exception SAXException if a parsing error is to be reported
     */
    //当读到开始标签的时候，会调用这个方法。 namespaceURI 就是命名空间， localName 是不带命名空间前缀的标签名， qName 是带命名空间前缀的标签名。通过 atts 可以得到所有的属性名和相应的值
    @Override
