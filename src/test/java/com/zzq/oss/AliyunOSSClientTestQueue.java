 ...
 *  阿里云对象存储OSS客户端
 * */
public class AliyunOSSClientTestQueue {

	private static ExecutorService executorService = Executors.newCachedThreadPool();
	
	private final static Queue<UploadFile> queue = new LinkedBlockingQueue<>();
    ...
    private final static String ACCESSKEYSECRET = "xx";
    ...
    private static boolean addQueue(UploadFile uploadFile){
    	return queue.offer(uploadFile);
    }
    ...
    public static void upload(String uploadType,String directoryPath,String contentOrPath){
        ...
        OSSClient oss_Client = new OSSClient(ENDPOINT, ACCESSKEYID, ACCESSKEYSECRET);
        new AliyunOSSClientTestQueue().createBucket(oss_Client);
        ...
        if(AliyunOSSClientTestQueue.STIRNGPYTE.equals(uploadType)){    		// 上传字符串
            try {
        ...
        }else if(AliyunOSSClientTestQueue.FILEPYTE.equals(uploadType)){// 上传本地文件
            ...
            if(found){
                ...
                try {
                    ...
                    System.out.println(AliyunOSSClientTestQueue.FILEPYTE+ " 上传网络流失败");
                    e.printStackTrace();
                }

            }
        }else if (AliyunOSSClientTestQueue.NETWORKTYTE.equals(uploadType)){//上传网络流
            try {
                ...
                for (File file2 : files) {
                    if (file2.isDirectory()) {
                       ...
                       // System.out.println("上传的文件 "+ file2.getAbsolutePath().replace(prefix, "") + " 转为 " + uploadPath.toString());
                        addQueue(new UploadFile(uploadPath.toString(),file2.getAbsolutePath()));
                        //上传文件
                        //AliyunOSSClientTestQueue.upload(AliyunOSSClientTestQueue.FILEPYTE,uploadPath.toString(),file2.getAbsolutePath());
                    }
                }
            }
        } else {
    ...
    public static void main(String[] args) {
    	...
    	long fileSize = queue.size();
    	while(!queue.isEmpty()){
    		final UploadFile f = queue.poll();
    		executorService.execute(new Runnable() {
				
				@Override
				public void run() {
					 System.out.println("上传的文件 "+ f.getFileAbsolutePath()+ " 转为 " + f.getOssPath());
				//	System.out.println("thread name ---- "+Thread.currentThread().getName());
					// TODO Auto-generated method stub
					AliyunOSSClientTestQueue.upload(AliyunOSSClientTestQueue.FILEPYTE,f.getOssPath().toString(),f.getFileAbsolutePath());
